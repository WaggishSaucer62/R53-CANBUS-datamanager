...
// 0x153 byte 0: brake pressure from 0-255 or so? or is it byte 1 or 2 (2nd and 3rd byte) or combined?
...
// 0x329 byte 0 random data? byte 1 a temperature?
...
// 0x545 byte 1 frequently goes between 0 and 255, byte 2 while driving increases roughly linearly? over time, before resetting to 0 at 255, but then a bit less
...
// (for example byte 0 for FL wheel), but no other bytes that look like they would combine, byte 6 seems to be high when engine off (127), and low when running
// (moves between 0 and 2)
